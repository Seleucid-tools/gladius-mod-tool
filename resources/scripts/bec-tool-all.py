...
# Default thread count: use CPU count, minimum 4
NUM_THREADS = max(4, os.cpu_count() or 4)

separator = ','
fileAtEnd = 0xC0000000
duplicateFlag = 0x2000000
checksumSize = 8

headerFile = "filelist.txt"
repackFile  = "repack-filelist.txt"

###########################################################################################################################################################

class RomSection():
    def __init__(self, name, hash, address,compressedSize, size):
    ...
    def unpackedCompressedSize(self):
        return self.CompressedSize & (~duplicateFlag)

    def storedSize(self):
        if self.unpackedCompressedSize() != 0 :
            return self.unpackedCompressedSize()
        else :
...
def scanFiles(dir):
    ...
    for root, directories, files in os.walk(dir, topdown=False):
        for name in files:
            if name.lower() == headerFile or name.lower() == repackFile:
                print("Skipping "+name)
                continue
...
def WriteSectionInFile(fByteArray, dirname, filename, debug=False):
    filename2 = dirname + filename
    # Use exist_ok=True so concurrent threads don't race on directory creation
    dirpath = os.path.dirname(filename2)
    if dirpath:
...
def _unpack_section(bec_filename, filedir, romSection):
    """Thread worker: open own file handle, read one section, decompress if needed, write to disk.
    Returns the number of decompressed bytes written."""
    with open(bec_filename, 'rb') as f:
        fileByteArray = ReadSection(f, romSection.DataOffset, romSection.DataSize)
    localByteArray = bytearray(fileByteArray)
    if len(localByteArray) > 0 and localByteArray[0] == 0x78:
        localByteArray = zlib.decompress(fileByteArray)
    WriteSectionInFile(localByteArray, filedir, romSection.FileName)
    return len(localByteArray)


def unpackBecArchive(filename, filedir, demobec, platform, num_threads=None, debug=False):
    print("Platform is : "+platform)
    print("Filename : "+filename)
    print("Filedir : "+filedir)
    file = open(filename, "rb+")
    header_output = unpackBecArchive2(file, filename, filedir, demobec, num_threads, debug)
...
def unpackBecArchive2(file, bec_filename, filedir, demobec, num_threads=None, debug=False):
    output = ""
    ...
    # Count duplicates (already known from header data)
    for romSection in RomSections:
        if romSection.IsDuplicate:
            totalDuplicates += 1

    # Phase: parallel decompress + write
    # Each worker opens its own file handle so there's no shared seek position.
    print("Unpacking " + str(len(RomSections)) + " files using " + str(num_threads) + " threads...")
    with ThreadPoolExecutor(max_workers=num_threads) as executor:
        future_to_section = {
            executor.submit(_unpack_section, bec_filename, filedir, rs): rs
            for rs in RomSections
        }
        completed = 0
        for future in as_completed(future_to_section):
            decompressed_size = future.result()  # re-raises any exception from the worker
            totalUnpacked += decompressed_size
            completed += 1
            if completed % 500 == 0:
                print("Unpack progress: " + str(completed) + "/" + str(len(RomSections)))

    # Build filelist output in the original sorted order (deterministic)
    for romSection in RomSections:
        ...
        dataLine += str(romSection.CompressedSize) + separator
...
def _load_and_compress(item, dir, compress):
    """Thread worker: read one file from disk and optionally compress it.
    Stores results directly on the item object."""
    item.OldSize = item.DataSize
    filepath = normpath(dir + "/" + item.FileName)
    ...
    if item.OldSize != item.DataSize:
        print(item.FileName + " size changed : " + str(item.OldSize) + " / " + str(item.DataSize))


def createBecArchive(dir, filename, becmap, demobec, platform, num_threads=None, debug=False):
    ...
    output += str(FileAlignment) + separator + str(NrOfFiles) + separator + str(HeaderMagic) + "\n"


    print("Creating file with : "+str(NrOfFiles)+" entries and file alignment : "+str(FileAlignment))
    ...
    # Sort by DataOffset; for entries sharing the same DataOffset (duplicates),
    # always place the primary (non-duplicate) before its duplicates so that
    # Phase 2 duplicate detection sees the original first.
    RomMap.sort(key=lambda x: (x.DataOffset, 1 if x.IsDuplicate else 0))

    print("Sorted Offset")

    # Phase 1: Parallel file read and compression
    # Workers set item.FileByteArray, item.CompressedSize, item.DataSize, item.OldSize
    print("Loading and compressing " + str(len(RomMap)) + " files using " + str(num_threads) + " threads...")
    with ThreadPoolExecutor(max_workers=num_threads) as executor:
        futures = [executor.submit(_load_and_compress, item, dir, compress) for item in RomMap]
        completed = 0
        for future in as_completed(futures):
            future.result()  # re-raises any exception from the worker
            completed += 1
            if completed % 500 == 0:
                print("Load/compress progress: " + str(completed) + "/" + str(len(RomMap)))

    # Phase 2: Sequential offset computation (must remain in DataOffset order;
    # duplicate detection depends on the previous item's OriginalDataOffset)
    lastItem = None
    for item in RomMap:
        duplicate = False
        ...
        if lastItem is not None:
           if lastItem.OriginalDataOffset == item.OriginalDataOffset and item.IsNew == False:
               #print("Adding duplicate item "+item.FileName+" at position "+str(currentAddr))
               duplicate = True
    ...
    for item in RomMap:
        ...
        output_rom.write(struct.pack('<I', item.CompressedSize))
        output_rom.write(struct.pack('<I', item.DataSize))

        dataLine =  item.FileName + separator
        dataLine += str(item.PathHash) + separator
        dataLine += str(item.DataOffset) + separator
        dataLine += str(item.CompressedSize) + separator
        dataLine += str(item.DataSize) + "\n"
        output += dataLine
    ...
    for item in RomMap:
        ...
        currentPosition = output_rom.tell()

        #handle items that had the same offset in the file
        if item.DataOffset < currentPosition:
            lastItem = item
            continue
...
if __name__ == "__main__":
    ...
    parser.add_argument("--threads", type=int, default=None, metavar="N", help="number of worker threads (default: CPU count, currently " + str(NUM_THREADS) + ")")
    ...
    if args.pack:
        outputText = createBecArchive(args.pack[0], args.pack[1], args.pack[2], demobec, args.platform, args.threads, debug)


        if not os.path.exists(os.path.dirname(repackFile)) and os.path.dirname(repackFile):
            os.makedirs(os.path.dirname(repackFile))
        #fheader = open(repackFile, 'w')
        #fheader.write(outputText)


    if args.unpack:
        unpackBecArchive(args.unpack[0], args.unpack[1], demobec, args.platform, args.threads, debug)
    #if args.scan:
